...
public class LambdaInterface {
    public static void main(String[] args) {
        ...
        // forEach를 쓰면 첨부터 끝까지 다 반복이기에 상황에 맞게 쓰기 (중간자가 없기때문에)
        ...
        System.out.println(newDataList2);

//        Stream<Integer> numStream = List.of(1,2,3,4).stream();    // 두가지방법
        /*
            스트림구성
            첫번째 단계 -> 스트림 생성
         ...
         */
        Stream<Integer> numStream = Stream.of(1,2,3,4);             // 이건 바로 stream으로 만드는거
        numStream.filter(num -> num % 2 == 0).forEach(n -> System.out.println(n));
        System.out.println("위에 코드는 stream으로  뽑은거");
        ...
        System.out.println("위에 코드는 for문으로 뽑은거");
    }
}
